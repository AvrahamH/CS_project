	...
	lw $a0, $zero, $imm2, $zero, 0, 0x100		# get R from address 
	mac $a0, $a0, $a0, $zero, 0, 0				# $a0 = R^2
	out $zero, $zero, $imm2, $imm1, 255, 21		# monitordata = 255
	add $t0, $zero, $zero, $zero, 0, -1			# i = -1
	add $ra, $imm1, $zero, $zero, end, 0
for_i:
	add $t0, $t0, $imm1, $zero, 1, 0			# i=0, i++
	beq $zero, $t0, $imm2, $imm1, end, 10		# for (i<10)
	add $t1, $zero, $zero, $zero, 0, -1			# j = -1
for_j:
	add $t1, $t1, $imm1, $zero, 1, 0			# j=0, j++
	beq $zero, $t1, $imm2, $imm1, for_i, 10		# for (j<10)
	sub $a1, $t0, $imm1, $zero, 128, 0			# a1 = x - x_center
	mac $a1, $a1, $a1, $zero, 0, 0				# a1 = a1^2 (x distance from center)
	sub $a2, $t1, $imm1, $zero, 128, 0			# a2 = y - y_center
	mac $a2, $a2, $a2, $zero, 0, 0				# a2 = a2^2 (y distance from center)
	add $a1, $a1, $a2, $zero, 0, 0				# a1 = (x - x_center)^2 + (y - y_center)^2
	bgt $zero, $a1, $a0, $imm1, for_j 			# if(a1 > R^2) return to beginning of loop
	mac $t2, $t0, $imm2, $t1, 0, 10				# buffer offset - i * 10 + j
