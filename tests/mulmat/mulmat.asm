...
.word 0x11f 32    
	...
	add $s2, $imm1, $zero, $zero, 0x120, 0		# s2 = c11 address (write to) 
	add $t0, $imm1, $zero, $zero, -1, 0			# i = -1
for_i:
	add $t0, $t0, $imm1, $zero, 1, 0			# i=0, i++
	bgt $zero, $t0, $a0, $imm1, end, 0			# for (i<4)
	add $t1, $zero, $zero, $zero,  0, 0			# j=0
for_j:		
	bgt $zero, $t1, $a0, $imm1, for_i, 0		# for (j < 4)
	add $t3, $zero, $imm1, $zero, 0, 0			# sum=0
	add $t2, $zero, $imm1, $zero, 0, 0			# k=0
	sll $s0, $t0, $imm1, $zero, 2, 0			# s0 = i * 4 offset of address of line i
for_k:		
	bgt $zero, $t2, $a0, $imm1, store, 0		# finishes for loop if k == 4 
	...
	add $s1, $s1, $t1, $zero, 0, 0				# calculate address second term in the sum [4*k+j]
	sw $s1, $sp, $imm1, $zero, -2, 0			
	lw $s1, $s1, $imm1, $zero, 0x110, 0			# loading of second term in the sum
	mul $s0, $s1, $s0, $zero, 0, 0				# mul of the two terms
	add $t3, $t3, $s0, $zero, 0, 0				# adds the products to previous calculations
...
store:
	sw $t3, $s2, $imm1, $zero, 0, 0				# store product result in memory
